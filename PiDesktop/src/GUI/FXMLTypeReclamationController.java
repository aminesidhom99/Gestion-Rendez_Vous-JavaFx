 ...
 * @author EMNA
 */
public class FXMLTypeReclamationController implements Initializable {
    //Type Reclamation
    @FXML
    ...
    @FXML
    private TableColumn<reclamation, user> UserCol;
    @FXML
    private TableColumn<reclamation, type_reclamation> TypeCol;
    @FXML
    private TableColumn<reclamation, Timestamp> dateCol;
    @FXML
    private TableColumn<reclamation, String> emailCol;
    @FXML
    private TableColumn<reclamation, Integer> telCol;
    @FXML
    private TableColumn<reclamation, String> cmntCol;
    @FXML
    ...
    private ChoiceBox<user> ajoutUser;
    @FXML
...
//Type Reclamation
        ...
        List<type_reclamation> lt = sp.afficher();
        List<reclamation> lr = sr.afficher();
        ObservableList<type_reclamation> obsListType = FXCollections.observableArrayList(sp.afficher());
...
TableColumn<reclamation, user> colUser = (TableColumn<reclamation, user>) table1.getColumns().get(0);
colUser.setCellValueFactory(cellData -> new SimpleObjectProperty<>(cellData.getValue().getId_user_id()));
colUser.setCellFactory(column -> new TableCell<reclamation, user>() {
    @Override
...
super.updateItem(item, empty);
    ...
    ServiceReclamation rec = new ServiceReclamation();
        ObservableList<reclamation> obListType = FXCollections.observableArrayList(rec.afficher());
    ...
    //haythem cha3mal
            ...
            for(reclamation r:lr){
              ...
              schart.add(new PieChart.Data("Reclamation Traité",nbt));
    ...
    private void SuppRec(ActionEvent event) {
        ...
        if (r != null) {
            ServiceReclamation rec = new ServiceReclamation();
            rec.supprimer(r.getId());
            ObservableList<reclamation> obsListRec = FXCollections.observableArrayList(rec.afficher());
...
public void traiter() {
 ...
 public void envoyeremail(String mail, String reponse, int id) {
    String to = mail;
    String from = "emna.mazlout@esprit.tn";
